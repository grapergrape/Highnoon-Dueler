...
- `static/js/presentation/` planned
  - Combat-only presentation timing and micro-interactions.
  - Fanned hand layout, hover previews, ghost-card play animations, and clean-round feedback.
  - Must not own duel rules, run persistence, screen navigation, or reward logic.
  - The implementation plan is tracked in [../combat-presentation-ux-plan.md](../combat-presentation-ux-plan.md).
...
- Keep gameplay data values in `data/`, duel rules in `duel/`, presentation timing in `presentation/`, and UI/canvas output in `ui/` + `rendering/`.
- Prefer one-way dependencies:
  - `app` -> `data` / `duel` / `ui` / `rendering` / `presentation`
  - `duel` -> `data` / `ui/combat-ui`
  - `presentation` -> pure duel preview helpers and DOM nodes generated by `ui`
  - `ui` and `rendering` can read duel/app state but should not mutate long-term run persistence directly.
- Presentation code may call callbacks passed in by `app.js`, but should not directly save runs, advance screens, or resolve combat rules.

## Combat Ownership

The active combat loop is documented in [../combat-rework-technical-plan.md](../combat-rework-technical-plan.md).
The next combat UX layer is documented in [../combat-presentation-ux-plan.md](../combat-presentation-ux-plan.md).
...
- `static/js/presentation/` planned
  - Owns the fanned combat hand, card hover preview timing, played-card ghost animation, input locking during presentation moments, and clean-round feedback.
  - Should call pure duel preview helpers for exact projected deltas instead of reimplementing card effects.
